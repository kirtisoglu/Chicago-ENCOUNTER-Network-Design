...
# 1. Generate an initial solution using Wilson's algorithm. (Get a rooted spanning tree. partition it somehow.)
 

# 2. Repeat until getting a valid partition. (Valid ??): 
#       Merge 2 subtrees into one subtree. Random 2 subtrees ??
#       Repartition the subtree until getting 2 valid subtrees. Valid ?? Possible secmeden yapabilir miyiz bu adimi?


# 3. Measure the score of the partition. Compare it with current solution. Score?


# 4. Save it as the best partition if there is an improvement in the score. (Otherwise?)

# Mix with single edge flips? 

# Count sp(G) by Kirchhoff's formula: 1/n times the product of nonzero eigenvalues of the matrix A(G)-D(G).
...
def uniform_spanning_tree(
    ...
    for node in graph.node_indices: 
        ...
        while u not in tree_nodes:  # iterates until we find a vertex u in tree_nodes
            next_node[u] = choice(list(graph.neighbors(u)))  # randomly select a neighbor of u
            u = next_node[u]  # replace u with its neighbor, because u is not in tree_nodes
        
        # when it stops, next_node[u] is in tree_nodes
...
# Starts contracting nodes from leaves. Population of a contracted leaf is moved to its parent. Once a leaf is found with ideal population, its edge is selected as cut edge.
# Key point: if a leaf does not have the ideal population yet, any vertex contracted to it must be in the same subtree with the leaf.
