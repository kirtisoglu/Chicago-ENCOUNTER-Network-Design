...
# districts are nearly populated. To do that, we partition the set of blocks into k sets of blocks such that 
# total populations of the sets are nearly equal. Each district will be the serving area of a facility (primary care center)
# We have k facilities (sets/districts) in total.

# The model I have designed for this problem is added to the second page of our overleaf file: https://www.overleaf.com/project/657cbea83593afcf83bbf52c
# The following code uses the pulp library for finding an exact solution.

# The following is a function f(x) that calls the input x=(blocks, locations, time), and returns an objective value, weights (sum of the populations in the sets) and solution (sets).

# You will need to call some inputs in your function. # The first step for us is to arrange these inputs. What inputs do we need? Once we are done with this,
...
# Time is a parameter $t$ that stops the code after $t$ seconds. The algorithm behind the library discovers the solution space, and does not stop 
# until checking all possible solutions in the space. If you have a time parameter t, the code stops after t seconds, and returns the best solution that is found so far.
...
def multiway_number_partitioning(blocks, locations, time):
    ...
    # Constraint: Each block is assigned to exactly one set. Add this constraint to the model by using problem+=. 
    ...
    # We define two new variables here for max and min weights of sets. 
    max_set_weight = LpVariable("max_set_weight", lowBound=0)
    min_set_weight = LpVariable("min_set_weight", lowBound=0)

    # Add constraints for max_set_weight and min_set_weight. For example: W_max \geq w(S_i) for all i.
    for j in locations.keys():
        problem += min_set_weight <= set_weight(j)
        problem += max_set_weight >= set_weight(j)

    # Add the objective function to the model: the difference between max and min set weights
